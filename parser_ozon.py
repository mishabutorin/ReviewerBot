...
class FeedbackParserOZN:
    ...
    def get_reviews_from_html(self):
        ...
        all_reviews = []
        characteristics = soup.find_all('div', class_='o6w ox4')

        for characteristic in characteristics:
            characteristic_elemets = characteristic.find_all('div', class_='ow7')
            for characteristic_elemet in characteristic_elemets:
                characteristic = characteristic_elemet.find('div', class_='w6o')
                if characteristic:
                    characteristic = characteristic.get_text(strip=True)
                else:
                    characteristic = 'Комментарий'

                text_element = characteristic_elemet.find('span', class_='wo6')
                if text_element:
    ...
    def get_item_name(self):
        ...
        soup = BeautifulSoup(html_content, 'html.parser')
        item_name = soup.find('a', class_='l9o')
